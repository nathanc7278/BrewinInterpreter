...
class Value:
    ...
    def __default_value_for_type(self, t):
        ...
        if t == Type.STRING:
            ...
            return False
        elif t == Type.OBJECT or t == Type.FUNCTION or t == Type.INTERFACE:
            return (
...
class Function:
    def __init__(self, func_ast, is_lambda=False, closure=None):
        ...
        self.interface = self.__interface(func_ast)

    def __interface(self, func_ast):
...
class Interpreter(InterpreterBase):
    ...
    def __create_interface_table(self, ast):
        ...
        for interface in ast.get("interfaces"):
            ...
            if len(interface_name) != 1:
                super().error(ErrorType.NAME_ERROR, "interface must be 1 letter")
            self.interfaces[interface_name] = []
            for field in interface.get("fields"):
                if field.get("name") in self.interfaces[interface_name]:
                    super().error(ErrorType.NAME_ERROR, "duplicate field in interface")
                self.interfaces[interface_name].append(field.get("name"))
    ...
    def __run_vardef(self, statement, block_def=False):
        ...
        if block_def:
            ...
            if var_type == Type.OBJECT:
                obj_value = self.env.get(name)
                self.env.bdef("selfo", obj_value)
        else:
            if not self.env.fdef(name, default_value):
                super().error(ErrorType.NAME_ERROR, "variable already defined")
            if var_type == Type.OBJECT:
                obj_value = self.env.get(name)
                self.env.fdef("selfo", obj_value)
        

    def __is_valid_interface(self, var_or_param, interface):
        if var_or_param.t != Type.OBJECT:
            return False
        if interface not in self.interfaces:
            super().error(ErrorType.TYPE_ERROR, "unknown interface")
        required = self.interfaces[interface]
        
        for field_name in required:
            if field_name not in var_or_param.v:
                return False
        return True
    ...
    def __run_assign(self, statement):
        ...
        if Type.get_type(dotted_name[-1]) == Type.INTERFACE and rvalue.v is not None:
            if rvalue.t != Type.OBJECT:
                super().error(ErrorType.TYPE_ERROR, "type mismatch in assignment")
            interface = dotted_name[-1][-1]
            if not self.__is_valid_interface(rvalue, interface):
                super().error(ErrorType.TYPE_ERROR, "rvalue does not satisfy interface")
        elif Type.get_type(dotted_name[-1]) != rvalue.t:
            if not(Type.get_type(dotted_name[-1]) == Type.FUNCTION and rvalue.v is None or \
                   Type.get_type(dotted_name[-1]) == Type.INTERFACE and rvalue.v is None):  
                super().error(ErrorType.TYPE_ERROR, "type mismatch in assignment")
    ...
    def __run_fcall(self, func_call_ast):
        ...
        func_val = self.__get_var_value(Element(InterpreterBase.QUALIFIED_NAME_NODE, name=fcall_name))
        ...
        if func_def is None:
            super().error(ErrorType.FAULT_ERROR, "function is nil")
        if len(func_def.formal_args) != len(actual_args):
            super().error(ErrorType.TYPE_ERROR, "wrong number of args")
        saved_env = self.env
        if func_def.is_lambda == True:
            self.env = Environment()
            for scope in func_def.closure.env:
                for scope_dict in scope:
                    new_scope = {}
                    for name, curr in scope_dict.items():
                        if curr.t == Type.OBJECT:
                            new_scope[name] = curr
                        else:
                            new_scope[name] = Value(curr.t, curr.v)
                    self.env.env.append([new_scope])
        else:
            self.env.enter_func()
        for formal, actual in zip(func_def.formal_args.keys(), actual_args):
            if formal[-1].isupper():
                ...
                if actual.t != Type.OBJECT:
                    super().error(ErrorType.TYPE_ERROR, "param does not satisfy interface")
                if not self.__is_valid_interface(actual, interface):
                    super().error(ErrorType.TYPE_ERROR, "param does not satisfy interface")
    ...
    def __run_return(self, funcdef, statement):
        ...
        if funcdef.interface is not None:
            if not self.__is_valid_interface(result_val, funcdef.interface):
                super().error(ErrorType.TYPE_ERROR, "return doesn't match interface")
        elif result_val.t != funcdef.return_type:
    ...
    def __eval_binary_op(self, kind, vl, vr):
        ...
        if kind == "==":
            if (tl == Type.OBJECT and tr == Type.OBJECT) or \
               (tl == Type.FUNCTION and tr == Type.FUNCTION):
                return Value(Type.BOOL, tl == tr and vl_val is vr_val)
            elif vl_val is None and vr_val is None:
    ...
    def __eval_convert(self, expr):
        ...
        if to_type == "int":
            if val.t == Type.INT:
                ...
                super().error(ErrorType.TYPE_ERROR, "cannot convert object/function to int")
            ...
            if val.t == Type.STRING:
                ...
                super().error(ErrorType.TYPE_ERROR, "cannot convert object/function to string")
            ...
            if val.t == Type.BOOL:
                ...
                super().error(ErrorType.TYPE_ERROR, "cannot convert object/function to bool")
        else:
    ...
    def __get_var_value(self, expr):
        ...
        if self.env.exists(dotted_name[0]):
            ...
            return value
        func = None
        for (name, param_type_sig), func_obj in self.funcs.items():
            if name == dotted_name[0]:
                func = func_obj
                break
        if not func:
            super().error(ErrorType.NAME_ERROR, "variable not defined")
        return Value(Type.FUNCTION, func)

    def __get_lambda(self, expr):
        closure = Environment()
        shadowed_args = [arg.get("name") for arg in expr.get("args")]
        for scope_dict in self.env.env[-1]:
            new_scope = {}
            for name, curr in scope_dict.items():
                if name not in shadowed_args:
                    if curr.t == Type.OBJECT:
                        new_scope[name] = curr
                    else:
                        new_scope[name] = Value(curr.t, curr.v)
            closure.env.append([new_scope])
        func = Function(expr, True, closure)
        return Value(Type.FUNCTION, func)
